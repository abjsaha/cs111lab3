...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		temp = f_pos - 2;
		//if (temp*OSPFS_DIRENTRY_SIZE >= dir_oi->oi_size)
		...
		/* EXERCISE: Your code here */
		od = ospfs_inode_data(dir_oi, temp * OSPFS_DIRENTRY_SIZE);
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	for (i=0; i<dir_oi->oi_size; i+=OSPFS_DIRENTRY_SIZE)
	{
		...
		if (entry->od_ino == 0) // empty entry is found
		{
			return entry;
		}
	}
	uint32_t size = dir_oi->oi_size;
	if (change_size(dir_oi, size+OSPFS_BLKSIZE) == 0)
	{
		entry = ospfs_inode_data(dir_oi, size);
		return entry;
	}
	return ERR_PTR(1);
}
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	/* EXERCISE: Your code here. */
	if(dst_dentry->d_name.len>OSPFS_MAXNAMELEN)//check name length
		return -ENAMETOOLONG;
